...
/**
 * PDVTab — Ponto de Venda integrado ao Supabase
 *
 * Props:
 *   companyId: string  — UUID da empresa (vem do AdminPage)
...
export function PDVTab({ companyId, onError }: PDVProps) {
  ...
  const handleCancelarCupom = () => { if (cart.length === 0) return; setCart([]); setSelectedCartId(null); showToast('Cupom cancelado') }
  ...
  const confirmVenda = () => {
    ...
    setCart([]); setSelectedCartId(null); setFinalModal(false)
  }

  // Número sequencial do cupom (simulado)
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1a3a5c] shrink-0 overflow-x-auto" style={{ background: '#071220' }}>
        ...
        {[
          { key: 'F1', label: 'Abertura',       fn: () => showToast('Abertura de caixa'),     danger: false },
          { key: 'F2', label: 'Fechamento',     fn: () => showToast('Fechamento de caixa'),   danger: false },
          { key: 'F3', label: 'Desc. item',     fn: handleDescItem,                           danger: false },
          { key: 'F4', label: 'Desc. total',    fn: handleDescTotal,                          danger: false },
          { key: 'F5', label: 'Cancelar item',  fn: handleCancelarItem,                       danger: true  },
          { key: 'F6', label: 'Cancelar cupom', fn: handleCancelarCupom,                      danger: true  },
          { key: 'F7', label: 'Desfazer desc.', fn: handleDesfazer,                           danger: false },
        ].map(({ key, label, fn, danger }) => (
          <button
            key={key}
            onClick={fn}
            className={`flex flex-col items-center min-w-[58px] px-2 py-1.5 rounded-lg border text-[10px] transition-colors
              ${danger
                ? 'border-red-900/60 hover:bg-red-950/60 text-red-400'
                : 'border-[#1a3a5c] hover:bg-[#1a3a5c] text-slate-400'}`}
          >
            <span className={`font-semibold ${danger ? 'text-red-400' : 'text-indigo-400'}`}>{key}</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── CUPOM (esquerda) ─────────────────────────────────────────── */}
        <div
          className="flex flex-col border-r border-[#1a3a5c] overflow-hidden"
          style={{ width: '55%', background: '#0d1b2a' }}
        >
          {/* Cabeçalho do cupom */}
          <div
            className="shrink-0 px-4 py-3 border-b border-[#1a3a5c]"
            style={{ background: '#091521' }}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-slate-500 bg-[#0f1e2e] px-2 py-1 rounded border border-[#1a3a5c]">
                  CUPOM #{cupomNum}
                </span>
          ...
          {/* Cabeçalho colunas */}
          <div
            className="grid items-center px-4 py-2 border-b border-[#1a3a5c] text-[11px] text-slate-500 font-medium shrink-0"
            style={{ gridTemplateColumns: '2fr 3fr 1fr 1fr 1.5fr', background: '#071220' }}
          >
            <span>Código</span>
            <span>Descrição</span>
          ...
          <div
            ...
            style={{ background: '#0d1b2a', scrollbarWidth: 'thin', scrollbarColor: '#1a3a5c transparent' }}
          >
            {cart.length === 0 ? (
              <div
              ...
              <div style={{ background: '#0d1b2a' }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: isSelected ? '#0f2a3e' : idx % 2 === 0 ? '#0d1b2a' : '#0c1825',
                      }}
                    >
                      ...
                      <span style={{ fontSize: 13, color: '#cbd5e1', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: 8 }}>
                        {item.name}
                        {item.discount > 0 && (
                          <span style={{ marginLeft: 6, fontSize: 10, background: 'rgba(194,65,12,0.2)', color: '#fb923c', padding: '2px 6px', borderRadius: 4 }}>
                            -{item.discount}%
                          </span>
                        )}
                      </span>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
                        ...
                        <button
                          ...
                          style={{ color: '#475569', fontSize: 11, lineHeight: 1, background: 'none', border: 'none', cursor: 'pointer', padding: 2, borderRadius: 3, transition: 'color 0.15s' }}
                ...
                {/* Linha de subtotal quando há descontos */}
          ...
          {/* Rodapé total */}
          <div
            className="shrink-0 border-t border-[#1a3a5c]"
          ...
          >
            ...
            {cart.length > 0 && (
              <div style={{ padding: '8px 16px', borderBottom: '1px solid rgba(26,58,92,0.3)', display: 'flex', gap: 16 }}>
                ...
                {cart.some(i => i.discount > 0) && (
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    <span style={{ fontSize: 10, color: '#475569' }}>c/ desconto</span>
        ...
        <div className="flex flex-col flex-1 overflow-hidden" style={{ background: '#091521' }}>
          ...
          {/* Grid de produtos */}
          {loading ? (
            <div className="flex-1 flex items-center justify-center" style={{ color: '#475569', fontSize: 13, gap: 8 }}>
            ...
            <div
              ...
              style={{
                ...
                alignContent: 'start',
                scrollbarWidth: 'thin',
                scrollbarColor: '#1a3a5c transparent',
              }}
            >
              {filtered.map(p => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      padding: 10,
                      borderRadius: 10,
                      border: isSelected ? '1px solid #6366f1' : '1px solid #1a3a5c',
                      background: isSelected ? '#0f2240' : '#0f1e2e',
                      cursor: 'pointer',
                      textAlign: 'left',
                  ...
                  >
                    {p.image ? (
                      <img
                        src={p.image}
                        alt={p.name}
                        style={{ width: '100%', height: 56, objectFit: 'cover', borderRadius: 6, background: '#1a3a5c' }}
                        onError={(e: any) => { e.target.style.display = 'none' }}
                      />
                    ) : (
                      <div style={{ width: '100%', height: 56, borderRadius: 6, background: '#0d1b2a', border: '1px dashed #1a3a5c', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#334155' }}>
                        Sem Foto
                      </div>
                    )}
                    <span style={{ fontSize: 12, color: '#cbd5e1', fontWeight: 500, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {p.name}
                    </span>
          ...
          <div className="shrink-0 border-t border-[#1a3a5c] p-3 flex flex-col gap-2" style={{ background: '#071220' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, background: '#0f1e2e', border: '1px solid #1a3a5c', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#e2e8f0', outline: 'none' }}
              />
              <button onClick={handleAddByCode} style={{ fontSize: 11, color: '#94a3b8', background: '#0f1e2e', border: '1px solid #1a3a5c', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
                Buscar
              </button>
              <button
                onClick={() => showToast('NFC-e requer configuração fiscal')}
                style={{ fontSize: 11, color: '#60a5fa', background: '#0a1520', border: '1px solid #1a3a5c', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', whiteSpace: 'nowrap' }}
              >
                🧾 NFC-e
              </button>
            </div>
      ...
      {toast && (
        <div style={{
          ...
          fontSize: 13,
          fontWeight: 500,
          background: toast.type === 'err' ? '#dc2626' : '#f97316',
          color: '#fff',
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#0d1b2a', border: '1px solid #1a3a5c', borderRadius: 16, padding: 24, width: 320, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <h2 style={{ fontSize: 15, fontWeight: 600, color: '#e2e8f0' }}>
              {discModal === 'item' ? 'Desconto no item' : 'Desconto no cupom'}
            </h2>
            <div style={{ position: 'relative' }}>
              <input
                autoFocus
                ...
                {(['dinheiro', 'pix', 'cartao'] as const).map(m => (
                  <button
                    ...
                    style={{
                      padding: '10px 0',
                      borderRadius: 8,
                      border: payMethod === m ? '1px solid #6366f1' : '1px solid #1a3a5c',
                      background: payMethod === m ? 'rgba(99,102,241,0.12)' : 'none',
                      color: payMethod === m ? '#a5b4fc' : '#64748b',
                      cursor: 'pointer',
                      fontSize: 13,
                      transition: 'all 0.15s',
                    }}
