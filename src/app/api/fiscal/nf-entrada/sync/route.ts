  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'SOAPAction': 'http://www.portalfiscal.inf.br/nfe/wsdl/NFeDistribuicaoDFe/nfeDistDFeInt',
...
async function extrairNotas(xmlResposta: string) {
  ...
  const retorno = body?.nfeDistDFeIntResponse?.nfeDistDFeIntResult?.retDistDFeInt

  if (!retorno) return []

  const cStat = retorno?.cStat
  // 137 = docto(s) localizado(s) | 138 = nenhum documento localizado
  if (cStat !== 137) return []
  ...
  for (const doc of lista) {
    ...
    try {
      ...
      // documento não é NF-e (pode ser evento, CT-e etc.) — ignora
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  // 1. Busca config fiscal
  const { data: cfg, error: cfgError } = await supabase
    .from('fiscal_config')
    .select('cnpj, cpf, cert_pfx_base64, cert_senha, ambiente, codigo_ibge')
    .eq('company_id', companyId)
    .single()

  if (cfgError || !cfg?.cert_pfx_base64) {
  ...
  // 2. Determina tipo de documento (CPF para teste, CNPJ para produção)
  const tipo  = cfg.cpf ? 'cpf'  : 'cnpj'
  const valor = cfg.cpf ? cfg.cpf : cfg.cnpj

  // cUFAutor = 2 primeiros dígitos do IBGE (ex: "25" para PB)
  const cUFAutor = cfg.codigo_ibge.substring(0, 2)

  // Recupera o último NSU salvo (para paginação)
  ...
  // 3. Monta e envia requisição SOAP
  const pfxBuffer = Buffer.from(cfg.cert_pfx_base64, 'base64')
  ...
  try {
    xmlResposta = await soapRequest(envelope, pfxBuffer, cfg.cert_senha)
  } catch (e: any) {
    return NextResponse.json(
  ...
  // 4. Extrai as notas do XML de resposta
  const notas = await extrairNotas(xmlResposta)

  // 5. Upsert no banco
  if (notas.length > 0) {
    await supabase.from('nf_entrada').upsert(
      notas.map(n => ({
    ...
    // Salva o último NSU retornado para a próxima consulta
    const maiorNSU = notas[notas.length - 1].nsu
    await supabase.from('nf_entrada_nsu').upsert(
