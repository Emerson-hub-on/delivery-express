...
/**
 * NfceLogsModal — modal F9 do PDV
 *
 * Exibe cupons com erro (rejeitado) ou não transmitidos (pendente)
 * Filtro por data, seleção múltipla via checkbox e reemissão em lote
 */
...
export function NfceLogsModal({ companyId, onClose, onRetentar }: Props) {
  ...
  const filtered = filter === 'todos'
    ? entries
    : entries.filter(e => e.nfce_status === filter)
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <input
              ...
              style={{
                fontSize: 11, color: C.txtPri, background: C.bg,
        ...
        {filtered.length > 0 && (
          <div style={{
            ...
            borderBottom: `1px solid ${C.border}`,
            display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0,
            background: selected.size > 0 ? '#f0f4ff' : C.surface,
            ...
            {selected.size > 0 && (
              <>
                ...
                {onRetentar && (
                  <button
                    onClick={handleRetentarSelecionados}
                    disabled={retrying}
                    style={{
                      marginLeft: 'auto',
        ...
        {!loading && filtered.length > 0 && (
          <div style={{
            display: 'grid',
            gridTemplateColumns: '36px 100px 110px 1fr 96px 28px',
            padding: '6px 20px',
            background: '#f4f6fa',
            borderBottom: `1px solid ${C.border}`,
            flexShrink: 0,
            ...
            {['', 'Pedido', 'Data/Hora', 'Chave / Motivo', 'Valor', ''].map((h, i) => (
              <span key={i} style={{
                fontSize: 10, fontWeight: 700, color: C.txtMuted,
                ...
                <div key={entry.id} style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ cursor: 'pointer' }} onClick={() => setExpanded(isOpen ? null : entry.id)}>
                      ...
                      {entry.nfce_numero && (
                        <p style={{ fontSize: 10, color: C.txtMuted, margin: 0 }}>
                          NFC-e {String(entry.nfce_numero).padStart(6, '0')}
                        </p>
                      )}
                    </div>

                    {/* Data */}
                    <p style={{ fontSize: 11, color: C.txtSec, cursor: 'pointer' }} onClick={() => setExpanded(isOpen ? null : entry.id)}>
                      {fmtData(entry.created_at)}
                    </p>

                    {/* Chave + motivo resumido */}
                    ...
                    <span
                      ...
                      style={{ color: C.txtMuted, fontSize: 11, textAlign: 'center', transition: 'transform 0.15s', transform: isOpen ? 'rotate(180deg)' : 'none', cursor: 'pointer' }}
                    >▼</span>
                  </div>
                    ...
                    <div style={{ padding: '0 20px 14px 52px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                      ...
                      {entry.nfce_chave && (
                        <div style={{ background: '#f8f9fb', borderRadius: 8, padding: '8px 12px', border: `1px solid ${C.border}` }}>
                          ...
                          <p style={{ fontSize: 11, color: C.txtPri, fontFamily: 'monospace', wordBreak: 'break-all', lineHeight: 1.6, margin: 0 }}>
                            {entry.nfce_chave}
                          </p>
                        </div>
                      )}
                      ...
                      <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <p style={{
                            ...
                            color: entry.nfce_status === 'rejeitado' ? '#991b1b' : '#92400e',
                            marginBottom: 3,
                          }}>
                            {entry.nfce_status === 'rejeitado'
                          ...
                          <p style={{
                            ...
                            color: entry.nfce_status === 'rejeitado' ? '#b91c1c' : '#78350f',
                          }}>
