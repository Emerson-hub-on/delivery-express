...
const SearchResults = ({ query, companyId, onClose }: SearchResultsProps) => {
    ...
    if (loading) return (
        <div className="px-4 py-3 text-sm text-zinc-400 border-t border-zinc-100 dark:border-zinc-700">
            Buscando...
        </div>
    )
            ...
            {filtered.map(product => (
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm text-zinc-800 dark:text-zinc-100 truncate">{product.name}</p>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">R$ {product.price.toFixed(2)}</p>
                    </div>
                </button>
            ))}
        </div>
...
export const Header = ({ slug, companyId }: HeaderProps) => {
    ...
    return (
        <>
            ...
            <header className="hidden md:flex w-full h-16 items-center border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 sticky top-0 z-40 shadow-sm">
                <div className="w-full max-w-7xl mx-auto px-6 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="w-9 h-9 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-zinc-100 shrink-0 flex items-center justify-center">
                            <Logo />
                        </div>
                    ...
                    {/* Status badge */}
                    <span className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 rounded-full px-2.5 py-1">
            ...
            {menuOpen && (
                <div className="hidden md:block fixed top-16 left-4 w-64 bg-white dark:bg-zinc-900 shadow-lg border border-zinc-100 dark:border-zinc-700 rounded-2xl z-50 p-4 flex flex-col gap-4">
            ...
            {searchOpen && (
                <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-zinc-900">
                    <div className="flex items-center px-3 gap-2 h-14 shrink-0 border-b border-zinc-100 dark:border-zinc-700">
                        <button onClick={handleCloseSearch}
                            className="p-1 text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-100 transition-colors shrink-0">
                            <X size={20} />
                        </button>
                        <input
                            ref={searchInputRef}
