        ...
        {availableSizes.map(v => {
          ...
          return (
            <div key={v} className="flex items-center gap-3">
              ...
              {/* Input de estoque — só aparece quando ativo */}
              {active ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-24 border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-black"
                  />
                  <span className="text-xs text-gray-400">
                    {entry.stock === null ? 'sem controle' : `uni`}
                  </span>
                </div>
              ) : (
                <span className="text-xs text-gray-300 italic">não disponível</span>
              )}
            </div>
          )
        })}
